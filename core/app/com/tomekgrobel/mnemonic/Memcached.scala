 ...
 */
object Memcached extends ReactiveMemcachedClientAPI {

  private lazy val client = loadClient

  private def loadClient(implicit app: Application): ReactiveMemcachedClientAPI = {
   ...
   */
  def add[T](key: String, value: T, exp: Duration)(implicit codec: Codec[T]): Future[Boolean] = client.add[T](key, value, exp)
   ...
   * The expiry time has default value of Duration.Inf (infinite duration).
   */
  def set[T](key: String, value: T, exp: Duration)(implicit codec: Codec[T]): Future[Unit] = client.set[T](key, value, exp)
   ...
   */
  def delete(key: String): Future[Boolean] = client.delete(key)
   ...
   */
  def get[T](key: String)(implicit codec: Codec[T]): Future[Option[T]] = client.get[T](key)
   ...
   */
  def compareAndSet[T](key: String, expecting: Option[T], newValue: T, exp: Duration)(implicit codec: Codec[T]): Future[Boolean] = client.compareAndSet[T](key, expecting, newValue, exp)
   ...
   */
  def transformAndGet[T](key: String, exp: Duration)(cb: (Option[T]) => T)(implicit codec: Codec[T]): Future[T] = client.transformAndGet[T](key, exp)(cb)
   ...
   */
  def getAndTransform[T](key: String, exp: Duration)(cb: (Option[T]) => T)(implicit codec: Codec[T]): Future[Option[T]] = client.getAndTransform[T](key, exp)(cb)
}
